...
resource "aws_key_pair" "samantro" {
  key_name   = "task1-key"
...
resource "aws_s3_bucket" "task1vishal" {
    bucket = "task1-vishal"
    acl    = "public-read"
...
resource "aws_cloudfront_distribution" "imgcloudfront" {
    origin {
        domain_name = "task1vishal.s3.amazonaws.com"
        origin_id = "S3-task1vishal" 
    ...
    default_cache_behavior {
        ...
        target_origin_id = "S3-task1vishal"
...
resource "aws_instance" "web" {
  ...
  connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = file("C:/Users/vishal verma/Downloads/task1-key.pem")
